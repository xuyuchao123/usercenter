...
public class PcBlacklistController
{
    ...
    @ApiImplicitParams({@ApiImplicitParam(name="carNum", value="被拉入黑名单的车牌号", required=false, dataType="String"),
            @ApiImplicitParam(name="createName", value="黑名单创建人的昵称", required=false, dataType="String"),
            @ApiImplicitParam(name="createMobile", value="黑名单创建人的手机号", required=false, dataType="String"),
    ...
    public JsonResultObj queryBlacklist(String carNum, String createName, String createMobile, String page, String size)
    {
        LOGGER.info("开始查询黑名单 carNum={} createName={} createMobile={} page={} size={}",carNum,createName,createMobile,page,size);
        JsonResultObj_Page resultObj_Page = null;
        try
        {
            List resList = blacklistService.getBlacklist(carNum,createName,createMobile,page,size);
//            resultObj = new JsonResultObj(true,blacklistVoList);
    ...
    @ApiImplicitParams({@ApiImplicitParam(name="carNum", value="被拉入黑名单的车牌号", required=true, dataType="String"),
    ...
    public JsonResultObj addBlacklist(String carNum, String reason, @ApiIgnore HttpServletRequest request)
    {
        LOGGER.info("开始新增黑名单 carNum={} reason={}", carNum,reason);
        JsonResultObj resultObj = null;
        PcUser pcUser = (PcUser) SecurityContextHolder.getContext().getAuthentication().getPrincipal();
        if(pcUser == null)
        {
            LOGGER.error("用户未登陆,新增黑名单失败！carNum={}",carNum);
        ...
        else
        {
            try
            {
                blacklistService.addBlacklist(carNum, reason,pcUser.getId());
        ...
        LOGGER.info("结束新增黑名单 carNum={} reason={}", carNum,reason);
        return  resultObj;
    }

    @PostMapping("/deleteBlacklist")
    @ApiOperation(value="删除黑名单")
    @ApiImplicitParam(name="carNum", value="被拉入黑名单的车牌号", required=true, dataType="String")
    @ApiResponses({@ApiResponse(code = 200,  message = "isSuccess=true：删除成功 isSuccess=false：删除失败，resMsg为错误信息")})
    public JsonResultObj deleteBlacklist(String carNum)
    {
        LOGGER.info("开始删除黑名单 carNum={}", carNum);
        JsonResultObj resultObj = null;
        try
        {
            blacklistService.removeBlacklist(carNum);
            resultObj = new JsonResultObj(true);
        }
        catch (Exception e)
        {
